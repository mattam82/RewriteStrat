...
Notation hrel A B := (A -> B -> Type).

(** * Morphisms.

   We now turn Mto the definition of [Proper] and declare standard instances.
...
Class Related {A : Type} {A' : Type} (R : hrel A A') (m : A) (m' : A') :=
  related_prf : R m m'.
...
Section Proper.
  ...
  Lemma eq_proper_proxy (x : A) : Proper (@eq A) x.
  Proof. firstorder. Qed.
  
  Lemma reflexive_proper_proxy `(Reflexive A R) (x : A) : Proper R x.
  Proof. firstorder. Qed.
  ...
  (C : A -> Type) (D : B -> Type)
  (R : hrel A B)
  (R' : forall (x : A) (y : B), R x y -> hrel (C x) (D y)) :
    hrel (forall x : A, C x) (forall x : B, D x) :=
    fun f g => forall x y (X : R x y), R' x y X (f x) (g y).

  (** The non-dependent, homogeneous version is an instance 
      where we forget dependencies. *)
  
  Definition respectful {A' B B'} (R : hrel A A') (R' : hrel B B') : hrel (A -> B) (A' -> B') :=
...
(** We favor the use of Leibniz equality or a declared reflexive crelation 
  when resolving [Proper], otherwise, if the crelation is given (not an evar),
  we fall back to [Proper]. *)
Hint Extern 1 (Proper _ _) => 
  class_apply @eq_proper_proxy ||
  class_apply @reflexive_proper_proxy : typeclass_instances.

(** Notations reminiscent of the old syntax for declaring morphisms. *)
Delimit Scope signature_scope with signature.

Arguments respectful_hetero {_ _ _ _ _} R'%signature _ _.
Arguments respectful {A A' B B'}%type (R R')%signature _ _.

Module ProperNotations.

  Notation " R ++> R' " := (@respectful _ _ _ _ (R%signature) (R'%signature))
    (right associativity, at level 55) : signature_scope.

  Notation " R ==> R' " := (@respectful _ _ _ _ (R%signature) (R'%signature))
    (right associativity, at level 55) : signature_scope.

  Notation " R --> R' " := (@respectful _ _ _ _ (flip (R%signature)) (R'%signature))
    (right associativity, at level 55) : signature_scope.
...
Section Relations.
  ...
  Lemma subrelation_proper `(mor : Proper R' m) 
  ...
  Global Instance proper_subrelation_proper_arrow :
    Proper (subrelation ++> eq ==> eq ==> arrow) (@Related A A).
...
Arguments transitivity {A R Transitive x} y {z} _ _. 
...
Section GenericInstances.
  ...
  Program Definition flip_proper A B C (RA : crelation A) (RB : crelation B)
          (RC : crelation C)
  ...
  Next Obligation.
  Proof with auto.
    split. intros ; apply (transitivity x0)...
    intros.
  ...
  Lemma Reflexive_partial_app_morphism A A' (R : crelation A) (R' : crelation A')
  ...
  Definition proper_flip_proper A (R : crelation A) `(mor : Proper R m) :
    Proper (flip R) m := mor.
...
Section Normalize.
  ...
  Lemma proper_normalizes_proper `(Normalizes R0 R1, Proper R1 m) : Proper R0 m.
  Proof.
    red in H, Proper0.
    apply H. assumption.
  Qed.
